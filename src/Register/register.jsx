    ...
    return (
        <div className="reg-container flex">
            ...
            <div className="right-container">
                <div className="title">
                    <h2>{t('registration.titles.main')}</h2>
                </div>
                <div className="form-container">
                    <form onSubmit={sendRegister}>
                        <div className="name-fields flex">
                            <div className="first-name">
                                <label>{t('registration.form.name-fields.first')}</label>
                                <input type="text" required onChange={(e)=>{setFname(e.target.value)}}/>
                            </div>
                            <div className="last-name">
                                <label>{t('registration.form.name-fields.last')}</label>
                        ...
                        <div className="emailPassword-fields grid">
                            ...
                            <label>{t('registration.form.c-password')}</label>
                            <input className={cpasswordError ? 'InputError':'passwordInput'}type="password" required onChange={(e)=>{setCPassword(e.target.value)}}/>
                        </div>
                        
                        {!loader ?
                            <button type='submit' disabled={btndisabled} className={btndisabled ? 'btn registerFormSubmit disabled':'btn registerFormSubmit'}>{t('registration.buttons.submit')} <FaArrowCircleRight className='btn-icon'/></button> :
